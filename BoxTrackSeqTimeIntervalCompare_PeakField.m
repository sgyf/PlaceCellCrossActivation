...
for DNUM = 1:numel(datefolderlist)        % DNUM = date number
    ...
    for SNUM = 1:numel(T)       % SNUM = session number
        ...
        while 1
            ...
            t_mean = t_evt./size(ep_time,1);   % mean time interval between each space bin
            
            % Find cells' peak place field on track
            %----------------------------------------------------------
            ...
            PeakField = struct('day',{date},'direction',C{2},'cellID_act',cellid_date,'cellorder',xi_peak_id,...
                'TimeInBins',t_mean,'PositionPix',figedge,'PeakField_avg',xi_peak);
